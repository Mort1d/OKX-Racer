...
class Accounts:
    ...
    def get_available_accounts(sessions: list):
        ...
        if config.PROXY['USE_PROXY_FROM_FILE']:
            ...
            if not accounts_from_json:
                raise ValueError("Нет аккаунтов в sessions/accounts.json")
    ...
    async def check_valid_accounts(self, accounts: list):
        ...
        logger.success(f"Валидные аккаунты: {len(valid_accounts)}; Invalid: {len(invalid_accounts)}")
    ...
    async def get_accounts(self):
        ...
        if not available_accounts:
            ...
            logger.success(f"Найдено доступных аккаунтов: {len(available_accounts)}.")
        ...
        if invalid_accounts:
            ...
            logger.info(f"Сохранено  {len(invalid_accounts)} невалидных аккаунтов в { config.WORKDIR}invalid_accounts.txt")
    ...
    async def create_sessions(self):
        while True:
            session_name = input('\nВведите имя сессии (нажмите Enter для выхода): ')
            ...
            if config.PROXY['USE_PROXY_FROM_FILE']:
                ...
                proxy = input("Введите прокси в формате login:password@ip:port (нажмите Enter, чтобы использовать без прокси): ")

            dict_proxy = self.parse_proxy(proxy) if proxy else None

            phone_number = (input("Введите номер телефона аккаунта: ")).replace(' ', '')
